...
class LogisticNormalPrior:
    ...
    def fit(self, X, y):
        ...
        with self.model:
            ...
            xi = pm.Bernoulli('xi', p=prob, shape=X.shape[1]) # inclusion probability for each variable
            alpha = pm.Normal('alpha', mu=0.0, sd=5.0) # Intercept
            beta = pm.Normal('beta', mu=0.0, sd=1.0, shape=X.shape[1]) # Prior for the non-zero coefficients
...
submission.to_csv("../submissions/logistic_normal_prior_varied_inclusion_probability_"+str(int(time.time()))+".csv", index=False)