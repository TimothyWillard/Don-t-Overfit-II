...
class LogisticNormalPrior:
    ...
    def fit(self, X, y):
        ...
        with self.model:
            ...
            beta = pm.Cauchy('beta', alpha=0, beta=coeff_shape, shape=X.shape[1]) # Prior for the non-zero coefficients
        ...
        with self.model:
            self.trace = pm.sample(4000, random_seed=4816, cores=1, progressbar=False, chains=1)
...
submission.to_csv("../submissions/logistic_cauchy_prior_"+str(int(time.time()))+".csv", index=False)