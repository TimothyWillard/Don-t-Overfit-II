...
expit = lambda x: (np.tanh(x) + 1.0)/2.0
...
print(res_gp)
with open("tanh_regression_L1_penalty_search_results_"+str(int(time.time()))+".pkl", "wb") as f:
    pkl.dump(res_gp, f)
...
submission.to_csv("../submissions/tanh_regression_L1_penalty_"+str(int(time.time()))+".csv", index=False)