...
class LogisticNormalPrior:
    ...
    def fit(self, X, y):
        ...
        with self.model:
            prob = pm.Beta("prob", alpha=1, beta=10)
            xi = pm.Bernoulli('xi', prob, shape=X.shape[1]) # inclusion probability for each variable
            alpha = pm.Normal('alpha', mu=0, sd=5) # Intercept
            std = pm.Exponential("std", lam=0.75)
            beta = pm.Normal('beta', mu=0, sd=std, shape=X.shape[1]) # Prior for the non-zero coefficients
...
submission.to_csv("../submissions/logistic_normal_prior_beta_probability_exponential_std_"+str(int(time.time()))+".csv", index=False)