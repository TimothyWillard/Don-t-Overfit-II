...
class LogisticNormalPrior:
    ...
    def fit(self, X, y):
        ...
        with self.model:
            ...
            p2 = pm.math.dot(np.exp(X), xi2*beta2)
...
#         test_score = expit(self.trace['alpha'][0] + np.dot(X, test_inc * test_beta))  
        ...
        y_pred = np.apply_along_axis(np.mean, 1, expit(self.trace['alpha'] + np.dot(X, np.transpose(estimate1)) + np.dot(np.exp(X), np.transpose(estimate2))))
...
submission.to_csv("../submissions/logistic_normal_prior_exps_"+str(int(time.time()))+".csv", index=False)